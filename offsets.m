...
void offsets_init() {
  if (@available(iOS 11.4, *)) {
    printf("this bug is patched in iOS 12 and above\n");
    exit(EXIT_FAILURE);
  } else if (@available(iOS 11.4, *)) {
    printf("offsets selected for iOS 11.4 or above\n");
    offsets = kstruct_offsets_11_4;
  } else if (@available(iOS 11.4, *)) {
    printf("offsets selected for iOS 11.4 to 11.4.1\n");
    offsets = kstruct_offsets_11_4;
  } else {
    printf("iOS version too low, 12 required\n");
    exit(EXIT_FAILURE);
  }
}
