...
void go() {
  ...
  // free all the other ntfl sockets:
  for (int i = 0; i < next_all_sock; i++) {
    close(sockets[i]);
  }
  
  printf("waiting for early ntfl gc...\n");
  // wait for the ntfl gc...
  for (int i = 0; i < 400; i++) {
    usleep(10000);
  }
  ...
  printf("waiting for second ntfl gc...\n");
  // wait for the ntfl gc...
  for (int i = 0; i < 400; i++) {
    usleep(10000);
  }
